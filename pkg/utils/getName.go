...
func GetName(respData *sheets.ValueRange) (string, error) {
	...
	ref, err := checkReferenceFromLinkForNameTask(link)
	if err != nil {
...
func CheckGenderInt(gender string) int {
	switch gender {
	case "м":
		return 2
	case "ж":
...
func checkReferenceFromLinkForNameTask(link string) (string, error) {
	cfg := config.Load()
	patternSlice := NewSiteMatcher()
	siteCell, err := patternSlice.GetCellForURLNameTask(link)
	if err != nil {
		return "", err
	}
...
func (sm *SiteMatcher) GetCellForURLNameTask(url string) (string, error) {
	for _, pattern := range sm.patterns {
		if pattern.Pattern.MatchString(url) {
			return pattern.Cell, nil
		}
	}
	return "A2", models.ErrorMatchingSite // или какое-то значение по умолчанию
}
